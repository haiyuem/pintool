# Notes
This is a fork of the Intel Pin tool, downloaded from sourceforge. 
Currently used for [Value Prediction study](https://github.com/haiyuem/Value-Prediction-for-Input-Data-Commonality). Modified code in source/tools/readMemVal
...
============
   ...
   o Linux kernel older than version 2.6.14 are no longer supported

   o Starting Pin 3.24, building Pin Tools on Linux is supported from GCC 5 and higher. 
     GCC 10 or higher is recommended, due to some limitations in earlier GCC versions.
...
============================================
   ...
   o Pin is built and distributed with its own OS-agnostic, compiler-agnostic runtime,
     ...
     3. A C++ runtime. Please note that the current version does not support exceptions and RTTI (Run-Time Type Information).
        C++11 is supported on Linux but not on Windows and macOS.
...
PinADX support
        ...
        Visual Studio 2017 up to Visual Studio 2022.
...
Changes added _After_ Pin 3.23 / 98579
===========================================
   o Pin on Linux now supports C++ 11 standard with the following known issues:
      - RTTI and exceptions are not supported (as in previous Pin versions)
      - Thread safety and Concurrency support library are not supported
        - The above applies also to std::atomic_xxx(std::shared_ptr) utility functions
        - Users must continue using Pin Thread API and LOCK primitives for multi threading scenarios and concurrency control
      - long double arithmetic (numerics library) is not supported on 32-bit systems and partially supported on 64-bit systems
        - The above also affects other functions using long double arithmetics (e.g. std::stold, std::to_string(long double))
      - locale is supported only for C, C.UTF-8, en_US.UTF-8 and POSIX formats
        - en_US.UTF-8 known issue: regex_traits::translate_nocase returns wrong value in certain cases
     Note:
      - Instructions for building Pin Tools with PinCRT have been modified (Linux only):
        - include directory <pin kit dir>/extras/stlport/include/ and <pin kit dir>/extras/libstdc++/include/ are now replaced by <pin kit dir>/extras/cxx/include/
        - linkage command line switch '-lstlport-dynamic' is now replaced by ‘-lc++ -lc++abi’
        For exact details please refer to "Build with PinCRT for Linux and OS-X" section in PinCRT documentation.
      - Building Pin Tools on Linux now requires GCC 5 or higher. GCC 10 or higher is recommended.
        Usage of GCC 8 or higher is recommended, due to some limitations in earlier GCC versions.     
   o Added new Pin API for syscall emulation:
       - VOID PIN_SetSyscallReturn( CONTEXT* ctxt, SYSCALL_STANDARD std, ADDRINT val):
          - Changes the return value of the syscall
       - VOID PIN_SetSyscallErrno( CONTEXT* ctxt, SYSCALL_STANDARD std, ADDRINT val):
          - Changes the error value of the syscall
   o Added support for OpenSuse LEAP 15.4, Fedora 36, Ubuntu 22.04 and RHEL 9
   o Added additional Pin tool stack overflow protection
      - Pin now detects stack overflow and asserts with proper message
   o Added Pin C++ enhancement (Windows, macOS)
      - std::stoull (C++11)
      - std::to_string (C++11)
      - std::ifstream constructor to receive std::string parameter (C++11)
   o Added ability to set max code cache size separately for 32-bit and 64-bit systems
      - use new knob -cc_memory_size_64 to override max code-cache size on 64-bit systems only


Changes added _After_ Pin 3.22 / 98547
===========================================
   o PIN CONTEXT API now supports tile registers (REG_TMM0-REG_TMM7) + REG_TILECONFIG. 
     These registers are part of AMX (Advanced Matrix Extension Instruction Set)
       o Documentation has been updated for PIN_SetContextRegval() and PIN_GetContextRegval().
   o Added supports for instrumenting applications with up to 8192 (previously up to 2048) threads on Windows 64-bit OS.
     Now both Linux 64-bit and Windows 64-bit OS support up to 8192 threads. On other OS, limit remains 2048 threads.
     Note PIN_MAX_THREADS definition is updated to reflect the above.
   o Reduced Pin-induced memory overhead. Pin-dedicated thread stack size is now set to 1MB (32-bit) / 2MB (64-bit).
   o Removed support for Linux Kernels older than 2.6.14.
   o Added support for OpenSuse LEAP 15.3

Changes added _After_ Pin 3.22 / 98537
===========================================
   o Added support for Fedora 35
     - Fixed a problem where Pin might crash when clone3() is called
   o Fixed a problem where Pin might crash with SIGSEGV at thread termination after Pin Tool uses PIN_CallApplicationFunction

    Changes added _After_ Pin 3.22 / 98528
===========================================
   o Added support for Ubuntu 21.10
     - Made several adaptations for behavior changes introduced by glibc 2.34 in Ubuntu 21.10  
   o Added Pin debugger plug-in for Visual Studio 2022 support

Changes added _After_ Pin 3.21 / 98484
===========================================
   o Added a new interface, ISCATTERED_MEMORY_REWRITE, by which the PinTool can rewrite vscatter/vgather 
     memory operands, and is passed to the analysis routine.
   o Added a new iarg, IARG_REWRITE_SCATTERED_MEMOP, that returns a pointer to the rewrite object ISCATTERED_MEMORY_REWRITE*.
   o Added a new function, INS_RewriteScatteredMemoryOperand, that switches memory operand rewrite on for vscatter/vgather.
     The rewrite feature needs both the analysis routine with IARG_REWRITE_SCATTERED_MEMOP and a call to this function for a functional rewrite.
   o Updated function INS_IsValidForIarg to support new iarg IARG_REWRITE_SCATTERED_MEMOP.
   o Added several new AMX APIs (Advanced Matrix Extension Instruction Set):
       o PIN_IsAmxActive()
       o TileCfg_GetTileRows()
       o TileCfg_GetTileBytesPerRow()
       o TileCfg_GetPaletteID()
   o Changes in IARG_REG_REFERENCE and IARG_REG_CONST_REFERENCE:
       o Analysis routines now recevies a UINT8* instead of PIN_REGISTER. This is a pointer to buffer holding the content of the
         requested register. Buffer size is the size of the requested register.
       o PIN_REGISTER union is now fully deprecated and has been removed from Pin.
       o If Amx is in init state (i.e., not active), a NULL pointer will passed to an analysis routine which receives tile register
         reference argument via IARG_REG_REFERENCE/IARG_REG_CONST_REFERENCE.
   o Added a new function, INS_OperandHasElements, that returns a boolean for whether an operand has elements.
   o Enhanced instruction multi-element operands inspection API to support AMX instructions:
        o A new function, IARG_MULTI_ELEMENT_OPERAND::ElementOffset(), that returns an element offset within a tile register.
   o Added support for debugging PIE (Position Indepndent executables) applications in Pin.
   o Added supports for instrumenting applications with up to 8192 (previously up to 2048) threads on Linux 64-bit OS. On other OS limit remains 2048 threads.
   o Added gettid() function to Pin-CRT. It is supported for all operating systems.
